...
def import_psd_results(filename:str):
    """
    Imports psd data results generated by matlab script.
    Matlab psd result are stored in the DAT/OUTPUT folder.

    Parameters:
        filename (str): filename of the matlab result.
    Returns:
        matlab_data (np.ndarray): imported data from the matlab csv result file.
    """
    #filename="MATLAB_PSD_res_EEG_Channel_5_C3_001_MolLud_20201112_1_c_preprocessed_499.998_Hz"
    matlab_data = np.genfromtxt("./DAT/OUTPUT/"+filename, delimiter=';',skip_header=1)
    PSD_fft_matlab= matlab_data[:,0:2]   #1ere colonne frequences, 2e resultats PSD
    PSD_p_matlab= matlab_data[:,2:4]     #1ere colonne frequences, 2e resultats PSD
    PSD_w_matlab= matlab_data[:,4:6]     #1ere colonne frequences, 2e resultats PSD
    return matlab_data
